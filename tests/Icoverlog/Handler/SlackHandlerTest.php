...
use Icoverlog\Handler\Slack\SlackRecord;
...
class SlackHandlerTest extends TestCase
{
    ...
    public function testWriteContent()
    {
        ...
        $this->assertMatchesRegularExpression('/username=Icoverlog/', $content);
    ...
    {
        $this->createHandler('myToken', 'channel1', 'Icoverlog', false);
        $this->handler->handle($this->getRecord(Level::Critical, 'test1'));
        fseek($this->res, 0);
        $content = fread($this->res, 1024);
    ...
    {
        $this->createHandler('myToken', 'channel1', 'Icoverlog', true, 'alien');
        $this->handler->handle($this->getRecord(Level::Critical, 'test1'));
        fseek($this->res, 0);
        $content = fread($this->res, 1024);
    ...
    private function createHandler($token = 'myToken', $channel = 'channel1', $username = 'Icoverlog', $useAttachment = true, $iconEmoji = null, $useShortAttachment = false, $includeExtra = false)
    {
        $constructorArgs = [$token, $channel, $username, $useAttachment, $iconEmoji, Level::Debug, true, $useShortAttachment, $includeExtra];
        $this->res = fopen('php://memory', 'a');
        $this->handler = $this->getMockBuilder('Icoverlog\Handler\SlackHandler')
