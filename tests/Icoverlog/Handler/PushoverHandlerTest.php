...
class PushoverHandlerTest extends TestCase
{
    ...
    {
        $this->assertMatchesRegularExpression('/token=myToken&user=myUser&message=test1&title=Icoverlog&timestamp=\d{10}$/', $content);
    }
    ...
    public function testWriteWithHighPriority()
    {
        ...
        $this->assertMatchesRegularExpression('/token=myToken&user=myUser&message=test1&title=Icoverlog&timestamp=\d{10}&priority=1$/', $content);
    }
    ...
    public function testWriteWithEmergencyPriority()
    {
        ...
        $this->assertMatchesRegularExpression('/token=myToken&user=myUser&message=test1&title=Icoverlog&timestamp=\d{10}&priority=2&retry=30&expire=25200$/', $content);
    }
    ...
    public function testWriteToMultipleUsers()
    {
        ...
        $this->assertMatchesRegularExpression('/token=myToken&user=userA&message=test1&title=Icoverlog&timestamp=\d{10}&priority=2&retry=30&expire=25200POST/', $content);
        $this->assertMatchesRegularExpression('/token=myToken&user=userB&message=test1&title=Icoverlog&timestamp=\d{10}&priority=2&retry=30&expire=25200$/', $content);
    }

    private function createHandler($token = 'myToken', $user = 'myUser', $title = 'Icoverlog')
    {
        ...
        $reflectionProperty = new \ReflectionProperty('Icoverlog\Handler\SocketHandler', 'connectionString');
